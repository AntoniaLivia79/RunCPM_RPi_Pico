...
#include "hardware/pico/waveshare_rp2040_zero_spi.h"
...
// =========================================================================================
  ...
  while (!Serial) {	// Wait until serial is connected
    ...
    a.neoPixelSetValue(0, MAX_INTENSITY, 0, 0, true);
    delay(sDELAY);
...
//   _puts("     running    on   Raspberry Pi [\e[1mPico-W\e[0m]\r\n");
  _puts("     compiled with   RP2040       [\e[1mv3.3.0\e[0m] \r\n");  
//  _puts("               and   ESP8266SdFat [\e[1mv2.1.1\e[0m] \r\n");
   _puts("               and   SDFat        [\e[1mv2.2.2\e[0m] \r\n");  
  _puts("----------------------------------------------\r\n");
  _puts("Revision             [\e[1m");
  _puts(GL_REV);
  _puts("\e[0m]\r\n");
...
// =========================================================================================
  SPI.setRX(0);   // MISO (pin number and not gpio number)
  SPI.setCS(1);   // Card Select (pin number and not gpio number)
  SPI.setSCK(2);  // Clock (pin number and not gpio number)
  SPI.setTX(3);   // MOSI (pin number and not gpio number)
...
void loop(void) {
  // digitalWrite(LED, HIGH^LEDinv);
  p.neoPixelSetValue(0, 0, 0, MAX_INTENSITY, true);
  delay(DELAY);
  // digitalWrite(LED, LOW^LEDinv);
  p.neoPixelClear();
  delay(DELAY);
